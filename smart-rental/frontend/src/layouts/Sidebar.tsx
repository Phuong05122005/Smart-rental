...
export const Sidebar = ({ isOpen, toggleSidebar }: { isOpen: boolean, toggleSidebar: () => void }) => {
  ...
  const menuItems = [
    ...
    { name: 'Quản lý nhà trọ', path: '/houses', icon: LayoutDashboard, roles: ['ADMIN', 'LANDLORD'] },
    { name: 'Dịch vụ', path: '/services', icon: LayoutDashboard, roles: ['ADMIN', 'LANDLORD'] },
    { name: 'Ghi chỉ số', path: '/meter-readings', icon: LayoutDashboard, roles: ['ADMIN', 'LANDLORD', 'STAFF'] },
